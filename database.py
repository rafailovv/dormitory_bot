...
class Database:
    ...
    def __init__(self, db_name):
        ...
        self.db = mysql.connector.connect(host=DATABASE_HOST,
                                          user=DATABASE_USER,
    ...
    def add_user(self, user_block, telegram_id, has_car=False, car_mark=None, car_number=None):
        ...
        try:
            self.cursor.execute(("INSERT INTO users (telegram_id, user_block_number, "
                                 "user_block_letter, car, role, registration_date) "
                                 "VALUES (%s, %s, %s, %s, %s, %s)"),
                                (int(telegram_id),
                                 user_block_number,
                                 user_block_letter,
            ...
            if has_car and car_mark and car_number:
                self.cursor.execute(("INSERT INTO users_cars (telegram_id, car_mark, car_number) "
                                     "VALUES (%s, %s, %s)"),
                                    (int(telegram_id), car_mark, car_number))
    ...
    def get_all_users_with_car(self):
        ...
        try:
            query = ("SELECT id, users.telegram_id, user_block_number, "
                     "user_block_letter role, car_mark, car_number, registration_date "
                     "FROM users "
                     "INNER JOIN users_cars ON users.telegram_id = users_cars.telegram_id;")
    ...
    def get_user_by_car_number(self, car_number):
        ...
        try:
            query = ("SELECT id, users.telegram_id, user_block_number, "
                     ...
                     "FROM users "
                     "JOIN users_cars "
                     "ON users.telegram_id = users_cars.telegram_id WHERE car_number = %s")
    ...
    def can_change_block(self, telegram_id):
        ...
        self.cursor.execute("SELECT last_block_change FROM users WHERE telegram_id = %s",
                            (telegram_id,))
    ...
    def change_block(self, telegram_id, new_block):
        ...
        self.cursor.execute(("UPDATE users SET user_block = %s, "
                            "last_block_change = %s WHERE telegram_id = %s"),
