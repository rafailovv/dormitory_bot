...
async def start_command(message: Message):
    ...
    start_message = (f"Здравствуйте, {user_name}. Бот успешно запущен и будет информировать"
                    "Вас о важных мероприятиях."
                    "Пожалуйста, не выключайте звук, чтобы не пропустить их)")
...
async def help_command(message: Message):
    ...
    commands_desc = {"start": "Запуск бота",
                     ...
                     "check_schedules":
                         "Проверяет файл с объявлениями и обновляет их (Только для админов)",
                     "announce_cars":
                         ("Уведомляет всех владельцев автомобилей, "
                          "проживающих в общежитии (Только для админов)"),
                     "announce_car":
                         ("Уведомяет жильца общежития с определенным "
                          "номером машины (Только для админов)"),
                     "cancel": "Отмена выполнение некоторых команд"}

    help_reply = ""
    for key, value in commands_desc.items():
        help_reply += f"<b>/{key}</b> - <em>{value}</em>\n"
...
async def info_command(message: Message):
    """ Команда /info """
    reply_message = ("Номер общежития: \n"
                     "<a href='tel:+79779437901'>+7 (977) 943-79-01</a>\n"
                    "Почта для отправки чеков об оплате общежития: "
                    "<a href='mailto:obsh4miit@yandex.ru'>obsh4miit@yandex.ru</a>\n"
...
async def announce_command(message: Message, state: FSMContext):
    ...
    if user:
        await message.answer(("Введите сообщение, "
                              "которое вы хотите передать всем жителям общежития!"))
...
async def announce_cars(message: Message, state: FSMContext):
    ...
    if user:
        await message.answer(("Введите сообщение, которое вы хотите передать "
                              "всем жителям общежития, у которых есть машина!"))
...
async def announce_car(message: Message, state: FSMContext):
    ...
    if user:
        await message.answer(("Введите номер машины, "
                              "владельцу которой вы хотите передать сообщение!"))
